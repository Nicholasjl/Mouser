...
> **说明：** MX Master、MX Anywhere、MX Vertical 和 G PRO 2 LIGHTSPEED 系列已有专用可视化热区覆盖层。其他 Logitech HID++ 设备仍可被识别并在 UI 中显示型号，使用通用 fallback 卡片。
...
- 一只支持的罗技 HID++ 鼠标（蓝牙或 USB 接收器均可）；MX Master、MX Anywhere、MX Vertical 和 G PRO 2 LIGHTSPEED 系列已有专用交互布局
