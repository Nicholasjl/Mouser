...
一个轻量、开源、**完全本地运行** 的 **Logitech Options+** 替代品，用于对罗技 HID++ 鼠标进行按键/手势重映射，并为主要支持型号提供专用交互布局，同时对更多罗技型号提供通用回退 UI。
...
- **设备感知 UI**：支持型号提供可点击热区的交互示意图；其他型号使用通用回退卡片
...
> **说明：** MX Master、MX Anywhere、MX Vertical、G PRO 2 LIGHTSPEED 和 G502 LIGHTSPEED 系列已有专用可视化热区覆盖层。其他 Logitech HID++ 设备仍可被识别并在 UI 中显示型号，使用通用 fallback 卡片。
...
- 一只支持的罗技 HID++ 鼠标（蓝牙或 USB 接收器均可）；MX Master、MX Anywhere、MX Vertical、G PRO 2 LIGHTSPEED 和 G502 LIGHTSPEED 系列已有专用交互布局
